...
/*int mult(struct no *aux1, struct no *aux2, int *j){
    type_queue aux_Q5;
    start(&aux_Q5);
    aux2=Q2.inicio;
    int i=0,k;
    if(aux1==NULL){
        ...
        *j=*j+1;
        k=*j;
        if(aux_Q6==NULL)
            aux_Q6=(type_queue*)malloc(sizeof(type_queue));
        else
            realloc(aux_Q6,*j);
        while(k>1){
            insert_inicio(&aux_Q6[*j-1],0);
            k--;
        }*
        aux_mult(aux1, aux2, aux_Q6, &i, 0);

        /*if(aux_Q6.inicio==NULL) {
            remove_fim(&aux_Q5, k);
            insert_inicio(&Q5,k);
            aux1=aux1->prox;
            return mult(aux1, aux2, j);
        }
        print_queue(aux_Q6[j-1]);
        //printf("%d",aux_Q5.inicio->info);
...
int main(){
    ...
    //aux_mult(Q1.inicio,Q2.inicio, &Q5,&i,0);
    //printf("value: %d", i);
    //printf("value: %d", mult(Q1.inicio, Q2.inicio, &i));
    //printf("i: %d\n", i);
    //sum(&Q1,&Q2,&Q3);
    //print_queue(&Q3);
    //sub();
